	...
	this.selectTool = function(toolName) {
		...
		for (var i = 0; i < this.tools.length; i++) {
			if (this.tools[i].name == toolName) {
				if(this.selectedTool != null && this.selectedTool.hasOwnProperty("removeToolOptions")){
					this.selectedTool.removeToolOptions();
				}
				if (this.tools[i] != null && this.tools[i].hasOwnProperty("addToolOptions")){
					this.tools[i].addToolOptions();
				}
